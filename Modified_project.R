...
RDOS_gaussian <- function(dataset=X, k=100, h=0.1)
{
  n <- nrow(dataset)
  d <- ncol(dataset)
  dataset <- as.matrix(dataset)
...
RDOS_laplace <- function(dataset=X, k=100, h=0.1)
{
  n <- nrow(dataset)
  d <- ncol(dataset)
  dataset <- as.matrix(dataset)
...
plotAUC_h <- function(X, Y, k)
{
  ...
  true_values <- Y[,c(33)]
  actual_values <- ifelse(true_values==1,0,1)
  
  Auc <- NULL
  for(h in X_h)
  {
    outlier_score <- RDOS_gaussian(dataset=X, k=100, h=0.1)
...
outlier_score1 <- ifelse(outlier_score>1.0, 1, 0)
...
true_values <- Y[,c(33)]
true_values <- ifelse(true_values==1,0,1)
